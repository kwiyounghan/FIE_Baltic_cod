...
## 02.1 load geno data
list.files(pattern=".bed")
f <- "cod3_filteredSNPs_gq20_dp8_ua_bial_miss08_meanDP10_hwe001_maf0.01_tempEBC_115_clean_noINV"
...
attributes(x)
#rm(geno.012)
#save(x, file="cod3_filteredSNPs_gq20_dp8_ua_bial_miss08_meanDP10_hwe001_maf0.01_tempEBC_115_clean_noINV_pcadapt.Rdata")
...
par(mfrow=c(1,1),oma=c(1,1,1,1),mar=c(5,4,3,2))
for (i in c(1)){
  var1 <- round(cumsum((x$singular.values[i])^2)*100,digits=2)
  var2 <- round(cumsum((x$singular.values[i+1])^2)*100,digits=2)
...
##################
###### Final figure
###### PCA plot all together with inversions and without inversions
##################
load(file="cod3_filteredSNPs_gq20_dp8_ua_bial_miss08_meanDP10_hwe001_maf0.01_tempEBC_115_clean_pcadapt.Rdata")
pca_all <- x

load(file="cod3_filteredSNPs_gq20_dp8_ua_bial_miss08_meanDP10_hwe001_maf0.01_tempEBC_115_clean_noINV_pcadapt.Rdata")

png(filename=paste0("cod3_filteredSNPs_gq20_dp8_ua_bial_miss08_meanDP10_hwe001_maf0.01_tempEBC_115_clean_all_pcadapt",".PC12.png",sep=""),width=4000,height=2000,res=300, units='px')
par(mfrow=c(1,2),oma=c(1,1,2,1))
## pca result with all sites first
i=1
  var1 <- round(cumsum((pca_all$singular.values[i])^2)*100,digits=2)
  var2 <- round(cumsum((pca_all$singular.values[i+1])^2)*100,digits=2)

  plot(pca_all$scores[,i],pca_all$scores[,i+1],
       col=pop_col, pch=19,
       cex=1.8,xlab=paste("PC",i," ",var1,"%",sep=""),ylab=paste("PC",i+1," ",var2,"%",sep=""),cex.lab=1.5)
  title(main="(a)",line=1,adj=0,cex=4)

## pca result without inversion second
  var1 <- round(cumsum((x$singular.values[i])^2)*100,digits=2)
  var2 <- round(cumsum((x$singular.values[i+1])^2)*100,digits=2)
...
#pcadapt
f <- "cod3_filteredSNPs_gq20_dp8_ua_bial_miss08_meanDP10_hwe001_maf0.01_tempEBC_115_clean"
file_name <- paste(f,"_INV",".PC1_2",".tiff",sep="")
...
for (i in huh){
  ...
  plot(x$scores[,1],x$scores[,2], col=pop_col,pch=19, cex=2,xlab=paste("PC1 ",var1,"%",sep=""),ylab=paste("PC2 ",var2,"%",sep=""),cex.lab=1.6)
  title(tit[i-25], cex.main=2)
}
